...
- **Recortes demográficos fora do corpo (decisão do usuário, 23/09/2026):** a lista de diferenças por sexo/cor ou raça/idade/instrução saiu da seção 9 — no lugar dela, uma nota explicando o motivo. Causa: o `survey` descarta réplicas bootstrap que ficam sem observação na célula ("N replicates gave NA results and were discarded"), o que pode subestimar o erro padrão e, por tabela, o CV que decide o que se publica. Medido em 2026T2 (Piauí, catálogo inteiro): 190 combinações indicador × território × recorte com descarte, mediana de 51,5 réplicas de 200, pior caso 91; concentrado em Cor ou raça (126), Instrução (35) e Faixa etária (19); **recorte Total: zero ocorrências**. Cruzando com a triagem: 745 linhas atingidas, 228 aprovadas pelo critério vigente. Documentado no `output/anexo_metodologico.md` §6.9, nas Limitações e no topo dos Apêndices A e C. O `01` continua estimando e testando esses recortes (o `09` é que deixou de ler `testes_significancia_<sufixo>.csv`); a composição da PIT (Tabela 7 e tabelas D) fica, por ser recorte Total e não ter descarte. Alternativa descartada por prazo: gravar as réplicas descartadas por estimativa e usá-las como critério na triagem (exigiria refazer a série, ~3 h).
- **Tabela A.1, definições dos indicadores (23/09/2026, usuário):** o Anexo A passou a trazer a tabela Dimensão × Medida × Indicador × Descrição de todos os indicadores, gerada pelo `tabela_definicoes()` do `09` (diretiva `<!-- @tabela tipo=definicoes -->`). Os textos ficam no vetor `DESCRICAO` do `09`, ao lado do `CATALOGO`, e o script **para com erro** se um id do catálogo não tiver descrição — indicador novo entra nos dois lugares. A ordem segue as dimensões do corpo, com "motivos" reunida a vulnerabilidade (a separação existe só por causa da matriz territorial).
- **Redistribuição de três indicadores entre dimensões (23/09/2026, usuário):** `Taxa_Subocupacao` saiu de "inserção" para "ocupacao" — a taxa composta de subutilização contém a subocupação, e separar as duas deixava o agregado numa seção e a parcela em outra. `Chefes_Familia_Desocupados` e `Conribuintes_Desocupados` saíram de "ocupacao" para "vulnerabilidade": não medem o nível de absorção do mercado, e sim quem é atingido pela desocupação (o desocupado sustenta o domicílio?) — mesma pergunta do desalento e do nem-nem. Fica 11/6/5/7/5 (ocupação, inserção, rendimento, vulnerabilidade, população) em vez de 12/7/5/5/5. Só o `CATALOGO` do `09` e o título da Tabela 5 no modelo mudaram; nenhuma fórmula, estimativa ou triagem foi tocada.
