...
do IBGE. Os indicadores são estimados por estrato amostral da própria pesquisa — incluindo os
estratos de 7 dígitos, o nível mais fino que o desenho da PNADC permite.

## Organização

Esse repositório se divide em duas partes, uma principal e uma auxiliar:

**Principal: Estimação dos indicadores** (`pipeline_trimestral.R`, `03_`) — baixa os
...
**Auxiliar: Geografia dos estratos** (`04_` a `08_`) — descobre *onde ficam* os
estratos que a parte principal usa como recorte, na resolução do setor censitário.
Ele serve somente com auxílio para tentar visualizar onde ficam localizado esses 
estratos no Piauí. **Isso é somente uma estimativa**: o IBGE publica o código do estrato nos microdados, mas
não a fronteira completa. Os detalhes estão em
[`output/reconstrucao_estratos.md`](output/reconstrucao_estratos.md).

Essa parte só auxilia na visualização espacial dos estratos e não impacta nos valores estimados
na parte principal. Não precisa ser rodado trimestral/anualmente.
...
60% no 1º/2026, 80% no 2º/2026 e 100% no 3º/2026. A versão atual serve como uma preparo
...
`pipeline_trimestral.R` grava o crosswalk em duas cópias: a corrente
...
- O restante (`01_run.R`, `01_processamento_dados.R`,
  ...
  `pnadc_estratosv1.R`, `pnadc_estratosv2.R`, `tentativa_cruzamento.R`, `GERAR_FIGURAS_TABELAS.R`) é resto.
